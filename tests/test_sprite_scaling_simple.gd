...
func test_sprite_scaling_property():
	...
	for iteration in range(iterations):
		...
		var scale = test_scaling_logic(dimensions.x, dimensions.y)
		
		# Calculate actual scaled dimensions
		var scaled_width = dimensions.x * scale.x
		var scaled_height = dimensions.y * scale.y
		...
		# Property 2: At least one dimension should be close to CELL_SIZE
...
func test_sprite_centering():
	...
	for iteration in range(iterations):
		...
		# Create a real sprite to test centering property
		var sprite = Sprite2D.new()
		
		# Property: Sprite2D nodes are centered by default
...
# Helper: Test the scaling logic directly without mock textures
# We'll test the mathematical properties of the scale_sprite_to_cell function
func test_scaling_logic(texture_width: float, texture_height: float) -> Vector2:
	# Replicate the scaling logic from SpriteManager.scale_sprite_to_cell()
	var max_dimension = max(texture_width, texture_height)
	if max_dimension == 0:
		return Vector2.ZERO
	var scale_factor = CELL_SIZE / max_dimension
